...
@Component({
  ...
  template: `<apx-chart [series]="series()" [chart]="chartConfig()" [xaxis]="xaxis()" [yaxis]="yaxis()" [stroke]="stroke()" [fill]="fill()" [dataLabels]="dataLabels()" [tooltip]="mergedTooltip()" [legend]="mergedLegend()" [grid]="mergedGrid()" [markers]="markers()" [title]="title()" [colors]="mergedColors()" (chartReady)="onChartReady($event)" />`,
...
export class UiLineChartComponent implements OnDestroy {
  ...
  readonly chartConfig = computed<ApexChart>(() => ({ ...this.defaults.chart, type: 'line', height: this.height() }));
