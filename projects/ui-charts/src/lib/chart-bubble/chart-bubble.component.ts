...
@Component({ selector: 'ui-bubble-chart', standalone: true, imports: [ChartComponent], template: `<apx-chart [series]="series()" [chart]="chartConfig()" [xaxis]="xaxis()" [yaxis]="yaxis()" [stroke]="stroke()" [fill]="fill()" [dataLabels]="dataLabels()" [tooltip]="mergedTooltip()" [legend]="mergedLegend()" [grid]="mergedGrid()" [markers]="markers()" [title]="title()" [colors]="mergedColors()" (chartReady)="onChartReady($event)" />`, styles: [chartHostStyles('ui-bubble-chart')], changeDetection: ChangeDetectionStrategy.OnPush, encapsulation: ViewEncapsulation.None })
export class UiBubbleChartComponent implements OnDestroy {
  ...
  readonly markers = input<ApexMarkers>({ size: 0 });
  ...
  readonly chartConfig = computed<ApexChart>(() => ({ ...this.defaults.chart, type: 'bubble', height: this.height() }));
