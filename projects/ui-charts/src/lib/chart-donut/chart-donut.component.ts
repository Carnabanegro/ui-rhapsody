...
export class UiDonutChartComponent implements OnDestroy {
  ...
  readonly plotOptions = input<ApexPlotOptions>({ pie: { donut: { size: '65%', labels: { show: true, name: { show: true }, value: { show: true } } } } });
  ...
  readonly chartConfig = computed<ApexChart>(() => ({ ...this.defaults.chart, type: 'donut', height: this.height() }));
