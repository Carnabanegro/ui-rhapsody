...
import type { ApexAxisChartSeries, ApexChart, ApexXAxis, ApexYAxis, ApexStroke, ApexFill, ApexDataLabels, ApexTooltip, ApexLegend, ApexGrid, ApexForecastDataPoints, ApexTitleSubtitle } from '../chart-inputs';
...
@Component({ selector: 'ui-range-area-chart', standalone: true, imports: [ChartComponent], template: `<apx-chart [series]="series()" [chart]="chartConfig()" [xaxis]="xaxis()" [yaxis]="yaxis()" [stroke]="stroke()" [fill]="fill()" [dataLabels]="dataLabels()" [tooltip]="mergedTooltip()" [legend]="mergedLegend()" [grid]="mergedGrid()" [forecastDataPoints]="forecastDataPoints()" [title]="title()" [colors]="mergedColors()" (chartReady)="onChartReady($event)" />`, styles: [chartHostStyles('ui-range-area-chart')], changeDetection: ChangeDetectionStrategy.OnPush, encapsulation: ViewEncapsulation.None })
export class UiRangeAreaChartComponent implements OnDestroy {
  ...
  readonly chartConfig = computed<ApexChart>(() => ({ ...this.defaults.chart, type: 'rangeArea', height: this.height() }));
