...
export class UiHeatmapChartComponent implements OnDestroy {
  ...
  readonly plotOptions = input<ApexPlotOptions>({ heatmap: { radius: 2 } });
  ...
  readonly chartConfig = computed<ApexChart>(() => ({ ...this.defaults.chart, type: 'heatmap', height: this.height() }));
